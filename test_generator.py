        ...
        for inputs in test_loader.data_loader:
            ...
            if opt.cuda:
                ...
                pre_clothes_mask = torch.FloatTensor(
                    (pre_clothes_mask.detach().cpu().numpy() > 0.5).astype(np.double)).cuda()
            else:
                ...
                pre_clothes_mask = torch.FloatTensor(
                    (pre_clothes_mask.detach().cpu().numpy() > 0.5).astype(np.double))
            ...
            if opt.cuda:
                ...
                warped_cm_onehot = torch.FloatTensor(
                    (warped_clothmask_paired.detach().cpu().numpy() > 0.5).astype(np.double))
